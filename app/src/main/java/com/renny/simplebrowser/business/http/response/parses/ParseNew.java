...
public class ParseNew implements IResultParse {
    @Override
    public Result parseResult(IApi iApi, String json, Type type) {
        if (TextUtils.isEmpty(json)) {//返回结果为空
            ...
            return Result.fail(ResultCode.LOCAL_EXCEPTION, "服务器数据返回异常");
        }
        try {
            JSONObject jsonObject = JSON.parseObject(json);
            JSONObject jsonObject1 = jsonObject.getJSONObject("result");
            if (jsonObject1 == null) {
                Logs.network.e("result等于null url=%s model=%s", iApi.getUrl(), type);
            }
            if (jsonObject1 != null) {

                boolean isSuccess = jsonObject1.getBoolean("success");
                String code = jsonObject1.getString("code");
                Object body = jsonObject1.getObject("data", Object.class);
                String msg = "";
